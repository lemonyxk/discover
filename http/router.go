...
func Router(s *router.Router[*http.Stream[server.Conn]]) {
	s.Group().Before(Middleware.localIP, Middleware.secret).Handler(func(handler *router.Handler[*http.Stream[server.Conn]]) {
	...
	s.Group().Before(Middleware.localIP, Middleware.secret, Middleware.isReady, Middleware.isMaster).Handler(func(handler *router.Handler[*http.Stream[server.Conn]]) {
	...
	s.Group().Before(Middleware.localIP, Middleware.isReady).Handler(func(handler *router.Handler[*http.Stream[server.Conn]]) {
		handler.Get("/ServerList").Handler(Action.ServerList)
		handler.Get("/Get/:key").Handler(Action.Get)
		handler.Get("/All").Handler(Action.All)
	})

	s.Group().Before(Middleware.localIP, Middleware.isReady, Middleware.isMaster).Handler(func(handler *router.Handler[*http.Stream[server.Conn]]) {
